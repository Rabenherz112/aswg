...
(function() {
    ...
    window.renderAppCard = function(config) {
        ...
        function getLinkAttrs(url, isInternal) {
        ...
        const categoriesHtml = app.categories ? app.categories.slice(0, config.maxCategoriesPerCard).map((category) =>
            `<span class="inline-block bg-badge-bg text-badge-text text-xs px-2 py-0.5 rounded-full">${category}</span>`
        ).join('') + (app.categories.length > config.maxCategoriesPerCard ? `<span class="inline-block bg-secondary text-secondary-text text-xs px-2 py-0.5 rounded-full">+${app.categories.length - config.maxCategoriesPerCard}</span>` : '') : '';

        const platformsHtml = app.platforms && app.platforms.length > 0 ? app.platforms.slice(0, config.maxPlatformsPerCard).map((platform) => {
        ...
        }).join('') + (app.platforms.length > config.maxPlatformsPerCard ? `<span class="inline-block bg-secondary text-secondary-text text-xs px-1.5 py-0.5 rounded">+${app.platforms.length - config.maxPlatformsPerCard}</span>` : '') : '';
        ...
        const detailsLink = `<a href="${detailsUrl}"${getLinkAttrs(detailsUrl, true)} class="text-link hover:text-link-hover font-medium">Details</a>`;
        ...
        card.innerHTML = `
            <div class="p-4 flex flex-col flex-grow">
                ...
                <div class="flex flex-wrap gap-2.5 text-xs mt-auto pt-2 border-t border-border">
                    ${websiteLink}${sourceLink}${demoLink}${detailsLink}${roadmapControl}
                </div>
            </div>
        `;
